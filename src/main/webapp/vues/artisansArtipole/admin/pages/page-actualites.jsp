...
<c:if test = "${not empty param.idAdh}">
    ...
    <div class="w-5/6 flex justify-center items-center gap-2">
      <div class="flex items-center p-1.5 bg-gray-200 dark:bg-gray-600 rounded-2xl gap-2 min-w-[35rem]">
        <div class="w-1/2 flex justify-center gap-2">
          <span class="text-2xl"><spring:message code="label.actualites"/></span>
        </div>
          ...
          <div class="flex items-center gap-2 mt-3">
            ...
            <div class="w-4/6">
              <c:choose>
                ...
                <c:otherwise>
                  <input type="hidden" name="idAdh" value="${param.idAdh}"/>
                  <input type="hidden" name="editAAActualite.adherent.id" value="${param.idAdh}"/>
                  <div class="text-white">&nbsp;<c:out value="${adherent.denomination}"/></div>
                </c:otherwise>
              </c:choose>
            </div>
          </div>
  ...
  <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
    ...
    <tbody>
      <c:forEach items="${itemsList}" var="item">
        <c:url value="/AA-edit-actualite" var="urlAAEdit"><c:param name="idActualite" value="${item.id}"/><c:if test="${not empty param.idAdh}"><c:param name="idAdh" value="${param.idAdh}"/></c:if></c:url>
        <c:url value="/AA-delete-actualite" var="urlAADelete"><c:param name="idActualite" value="${item.id}"/><c:if test="${not empty param.idAdh}"><c:param name="idAdh" value="${param.idAdh}"/></c:if></c:url>
        <tr class="<c:choose>
