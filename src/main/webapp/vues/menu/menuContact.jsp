  ...
  <div class="overflow-y-auto flex flex-col justify-between py-4 px-3 bg-gray-200 min-h-screen dark:text-gray-50 dark:bg-gray-800">
    <ul class="space-y-2">
      ...
      <li>
        <a href=<c:url value="/dashboard"/>
           class="<c:choose><c:when test = "${pageType == 'DASHBOARD'}">bg-green-600 hover:bg-green-700 </c:when><c:otherwise> hover:bg-gray-100 dark:hover:bg-gray-600 </c:otherwise></c:choose> flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-gray-50">
      ...
      <li>
        <a href=<c:url value="/listeAdherents"/>
           class="<c:choose><c:when test = "${pageType == 'LIST_ADHERENT' || pageType == 'CREATE_ADHERENT'}">bg-green-600 hover:bg-green-700 </c:when><c:otherwise> hover:bg-gray-100 dark:hover:bg-gray-600 </c:otherwise></c:choose> flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-gray-50">
      ...
      <li>
        <a href=<c:url value="/listeContact"/>
           class="<c:choose><c:when test = "${pageType == '' || pageType == 'LIST_CONTACT'}">bg-green-600 hover:bg-green-700 </c:when><c:otherwise> hover:bg-gray-100 dark:hover:bg-gray-600 </c:otherwise></c:choose> flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-gray-50">
      ...
      <li>
        <a href="listeRetraite"
           class="<c:choose><c:when test = "${pageType == '' || pageType == 'LIST_CONTACT_RETRAITE'}">bg-green-600 hover:bg-green-700 </c:when><c:otherwise> hover:bg-gray-100 dark:hover:bg-gray-600 </c:otherwise></c:choose> flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-gray-50">
             <svg class="w-6 h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg">
               <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <a href="listeClubFemmes"
           class="<c:choose><c:when test = "${pageType == '' || pageType == 'LIST_CONTACT_CLUB_FEMMES'}">bg-green-600 hover:bg-green-700 </c:when><c:otherwise> hover:bg-gray-100 dark:hover:bg-gray-600 </c:otherwise></c:choose> flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-gray-50">
             <svg class="w-6 h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg">
               <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                     d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
    ...
    <ul class="mb-20 flex flex-col">
      ...
      <sec:authorize access="hasAnyRole('ROLE_ADMIN', 'ROLE_EXPORT')">
        <li>
          <c:choose>
            <c:when test = "${pageType == '' || pageType == 'LIST_CONTACT_CLUB_FEMMES'}">
              <c:url value="/exportClubFemmeList" var="urlListe"/>
            </c:when>
            <c:when test = "${pageType == '' || pageType == 'LIST_CONTACT_RETRAITE'}">
          ...
          <form:form class="h-6" id="exportListForm" method="post" modelAttribute="criteria" action="${urlListe}">
            ...
            <a href="javascript:;" onclick="parentNode.submit();" 
              ...
              <div class="ml-3 whitespace-nowrap text-sm"><spring:message code="menu.exportContactList"/></div>
            </a>
          </form:form>
        </li>
      </sec:authorize>
      ...
      <sec:authorize access="hasAnyRole('ROLE_ADMIN', 'ROLE_RETRAITE_EDIT')">
        <c:if test = "${pageType == '' || pageType == 'LIST_CONTACT_RETRAITE'}">
          <li>
            <a href=<c:url value="/addContactRetraite"/> 
               class="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg hover:bg-gray-100 dark:text-gray-50 dark:bg-gray-800 dark:hover:bg-gray-600">
              <svg class="w-6 h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path>
              </svg>
              <div class="ml-3 whitespace-nowrap text-sm"><spring:message code="menu.addContactRetraite"/></div>
            </a>
          </li>
        </c:if>
      </sec:authorize>
      <sec:authorize access="hasAnyRole('ROLE_ADMIN', 'ROLE_CLUB_FEMMES_EDIT')">
        <c:if test = "${pageType == '' || pageType == 'LIST_CONTACT_CLUB_FEMMES'}">
          <li>
            <a href=<c:url value="/addContactClubFemme"/>
               class="<c:if test = "${pageType == '' || pageType == 'CREATE_ADHERENT'}"> text-green-500 font-bold </c:if> flex items-center p-2 text-base font-normal text-gray-900 rounded-lg hover:bg-gray-100 dark:text-gray-50 dark:bg-gray-800 dark:hover:bg-gray-600">
                 ...
                 <div class="ml-3 whitespace-nowrap text-sm"><spring:message code="menu.addContactClubFemmes"/></div>
            </a>
          </li>
        </c:if>
      </sec:authorize>  
